...
_C.MODEL.MODEL_ROOT = "/home/ch7858/Mixed_Prompt/models"  # root folder for pretrained model weights (changed here!)
...
# ----------------------------------------------------------------------
_C.MODEL.PROMPT_AS = CfgNode()
_C.MODEL.PROMPT_AS.NUM_TOKENS = 5
_C.MODEL.PROMPT_AS.LOCATION = "prepend"
# PROMPT_AS initalizatioin: 
    # (1) default "random"
    # (2) "final-cls" use aggregated final [cls] embeddings from training dataset
    # (3) "cls-nolastl": use first 12 cls embeddings (exclude the final output) for deep PROMPT_AS
    # (4) "cls-nofirstl": use last 12 cls embeddings (exclude the input to first layer)
_C.MODEL.PROMPT_AS.INITIATION = "random"  # "final-cls", "cls-first12"
_C.MODEL.PROMPT_AS.CLSEMB_FOLDER = ""
_C.MODEL.PROMPT_AS.CLSEMB_PATH = ""
_C.MODEL.PROMPT_AS.PROJECT = -1  # "projection mlp hidden dim"
_C.MODEL.PROMPT_AS.DEEP = False # "whether do deep PROMPT_AS or not, only for prepend location"


_C.MODEL.PROMPT_AS.NUM_DEEP_LAYERS = None  # if set to be an int, then do partial-deep PROMPT_AS tuning
_C.MODEL.PROMPT_AS.REVERSE_DEEP = False  # if to only update last n layers, not the input layer
_C.MODEL.PROMPT_AS.DEEP_SHARED = False  # if true, all deep layers will be use the same PROMPT_AS emb
_C.MODEL.PROMPT_AS.FORWARD_DEEP_NOEXPAND = False  # if true, will not expand input sequence for layers without PROMPT_AS
# how to get the output emb for cls head:
    # original: follow the orignial backbone choice,
    # img_pool: image patch pool only
    # prompt_pool: PROMPT_AS embd pool only
    # imgprompt_pool: pool everything but the cls token
_C.MODEL.PROMPT_AS.VIT_POOL_TYPE = "original"
_C.MODEL.PROMPT_AS.DROPOUT = 0.0
_C.MODEL.PROMPT_AS.SAVE_FOR_EACH_EPOCH = False
_C.MODEL.PROMPT_AS.FT_PT_MIXED = False # if true, will do mixed pt+ft training

_C.MODEL.PROMPT_AS.SINK_LOCATION = "prepend" # prepend or append
_C.MODEL.PROMPT_AS.SINK_NUMBER = 20 # the total number of sink tokens
_C.MODEL.PROMPT_AS.SINK_LAYERS = 1 # 1, 2, 4 (total layers: 12)
_C.MODEL.PROMPT_AS.SINK_RESIDUAL = False
_C.MODEL.PROMPT_AS.SINK_RESIDUAL_INTERVAL = 2 # 2, 4, 6 (only work when RESIDUAL is True)

# ----------------------------------------------------------------------
# Prompt with fourier
# ----------------------------------------------------------------------
_C.MODEL.PROMPT_FOURIER = CfgNode()
_C.MODEL.PROMPT_FOURIER.NUM_TOKENS = 5
_C.MODEL.PROMPT_FOURIER.LOCATION = "prepend"
# PROMPT_FOURIER initalizatioin: 
    # (1) default "random"
    # (2) "final-cls" use aggregated final [cls] embeddings from training dataset
    # (3) "cls-nolastl": use first 12 cls embeddings (exclude the final output) for deep PROMPT_FOURIER
    # (4) "cls-nofirstl": use last 12 cls embeddings (exclude the input to first layer)
_C.MODEL.PROMPT_FOURIER.INITIATION = "random"  # "final-cls", "cls-first12"
_C.MODEL.PROMPT_FOURIER.CLSEMB_FOLDER = ""
_C.MODEL.PROMPT_FOURIER.CLSEMB_PATH = ""
_C.MODEL.PROMPT_FOURIER.PROJECT = -1  # "projection mlp hidden dim"
_C.MODEL.PROMPT_FOURIER.DEEP = False # "whether do deep PROMPT_FOURIER or not, only for prepend location"


_C.MODEL.PROMPT_FOURIER.NUM_DEEP_LAYERS = None  # if set to be an int, then do partial-deep PROMPT_FOURIER tuning
_C.MODEL.PROMPT_FOURIER.REVERSE_DEEP = False  # if to only update last n layers, not the input layer
_C.MODEL.PROMPT_FOURIER.DEEP_SHARED = False  # if true, all deep layers will be use the same PROMPT_FOURIER emb
_C.MODEL.PROMPT_FOURIER.FORWARD_DEEP_NOEXPAND = False  # if true, will not expand input sequence for layers without PROMPT_FOURIER
# how to get the output emb for cls head:
    # original: follow the orignial backbone choice,
    # img_pool: image patch pool only
    # prompt_pool: PROMPT_FOURIER embd pool only
    # imgprompt_pool: pool everything but the cls token
_C.MODEL.PROMPT_FOURIER.VIT_POOL_TYPE = "original"
_C.MODEL.PROMPT_FOURIER.DROPOUT = 0.0
_C.MODEL.PROMPT_FOURIER.SAVE_FOR_EACH_EPOCH = False
_C.MODEL.PROMPT_FOURIER.FT_PT_MIXED = False # if true, will do mixed pt+ft training

_C.MODEL.PROMPT_FOURIER.FOURIER_TYPE = "fft" # the type of fourier transform (default is fast fourier transform)
_C.MODEL.PROMPT_FOURIER.FOURIER_DIMENSION = "all" # all, sequence or hidden
_C.MODEL.PROMPT_FOURIER.FOURIER_PERCENTAGE = 0.5
_C.MODEL.PROMPT_FOURIER.FOURIER_NUM = 100 # the total number of fourier prompts
_C.MODEL.PROMPT_FOURIER.FOURIER_FIRST_LAYER = True
_C.MODEL.PROMPT_FOURIER.FOURIER_ADDITION = False
_C.MODEL.PROMPT_FOURIER.FOURIER_LOCATION = 'prepend' # append, prepend or random
_C.MODEL.PROMPT_FOURIER.FOURIER_ADDITION_NUM = 50 # (only work when FOURIER_ADDITION is True)

_C.MODEL.PROMPT_FOURIER.POSITION_EMBEDDINGS = True
_C.MODEL.PROMPT_FOURIER.MIXED = False
_C.MODEL.PROMPT_FOURIER.FOURIER_HALF = "all"

_C.MODEL.PROMPT_FOURIER.VIS = False
...
_C.DATA.FOURIER = False
_C.DATA.FOURIER_PATH = './mmclassification/data/imagenet/train'
_C.DATA.FOURIER_LB = 0.1
